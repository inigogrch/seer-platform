...
export default function SavedStoriesPage() {
  ...
  const collections: Collection[] = [
    { id: 'all', name: 'All Stories', count: 6, color: 'bg-white/10' },
    { id: 'ai-research', name: 'AI Research', count: 2, color: 'bg-blue-500/20' },
    { id: 'product-insights', name: 'Product Insights', count: 2, color: 'bg-green-500/20' },
    { id: 'industry-trends', name: 'Industry Trends', count: 2, color: 'bg-purple-500/20' }
  ]
      ...
      <main className="w-full px-6 py-8">
        ...
        <div className={`mb-8 ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}>
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-4xl font-serif font-bold text-white tracking-tight">Saved Stories</h1>
                <p className="text-white/70 mt-1">Your curated collection of AI news and insights</p>
              </div>
            </div>
            <div className="px-4 py-2 seer-glass rounded-lg">
              <div className="text-sm text-white/70">Total Saved</div>
              <div className="text-lg font-bold text-white">{filteredStories.length} stories</div>
            </div>
          </div>
          ...
          <div className="flex items-center space-x-3 mb-6 overflow-x-auto pb-2">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-xl font-medium whitespace-nowrap transition-all ${
                  selectedCollection === collection.id
                    ? 'bg-white/20 text-white shadow-lg shadow-seer-primary/20 border border-white/30'
            ...
            <button
              ...
              className="px-4 py-2 seer-glass text-white/80 hover:bg-white/10 rounded-xl transition-all inline-flex items-center space-x-2 whitespace-nowrap"
            >
            ...
            <div className="seer-card p-4 mb-6">
              <div className="flex items-center space-x-3">
                ...
                <input
                  ...
                  className="flex-1 px-4 py-2 seer-input"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredStories.map((story, index) => (
            <article
              ...
              className={`group seer-card cursor-pointer ${!story.isRead ? 'ring-2 ring-seer-primary/30' : ''} ${isVisible ? 'animate-fade-in-up' : 'opacity-0'}`}
            ...
            >
              ...
              {story.image && (
                <div className="relative h-40 w-full rounded-t-2xl overflow-hidden bg-white/5">
                  ...
                  <div className="absolute top-3 right-3 flex items-center space-x-2">
                    {!story.isRead && (
                      <span className="px-2 py-1 bg-seer-primary/80 backdrop-blur-md text-white text-xs font-semibold rounded-full">
                        Unread
                      </span>
                    )}
                    {story.trending && (
                      <div className="px-2 py-1 bg-orange-500/80 backdrop-blur-md text-white text-xs font-semibold rounded-full inline-flex items-center space-x-1">
                  ...
                  <div className="absolute top-3 left-3">
                    <div className="flex items-center space-x-0.5">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-4 h-4 ${i < story.rating ? 'fill-yellow-400 text-yellow-400' : 'text-white/30'}`}
                        />
                      ))}
                    </div>
                  </div>
              ...
              <div className="p-5">
                ...
                {/* Notes */}
                {story.notes && (
                  <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-2.5 mb-3">
                    <div className="flex items-start space-x-2">
                      <StickyNote className="w-3.5 h-3.5 text-yellow-400 mt-0.5 flex-shrink-0" />
                      <p className="text-xs text-yellow-200 line-clamp-2">{story.notes}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {story.tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="inline-flex items-center space-x-1 px-2 py-0.5 bg-white/10 text-white/80 text-xs rounded font-medium"
                    >
        ...
        {filteredStories.length === 0 && (
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-serif font-semibold text-white mb-2">No stories found</h3>
            <p className="text-white/70">Try adjusting your search or collection filter.</p>
          </div>
        )}
      </main>
    </div>
